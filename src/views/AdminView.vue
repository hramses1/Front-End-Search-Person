...
/* ─── Estadisticas: tabla de usuarios + serie diaria ───────────
   Dos endpoints nuevos de admin. sample_complete en ambos avisa que el
   backend corto la ventana de peticiones por volumen: las cifras que
   dependen de ese log (total_requests, last_activity, peticiones_totales,
   peticiones_por_tipo) quedan parciales, no falsas del todo.
   ───────────────────────────────────────────────────────────── */
...
watch(pestana, (v) => {
  ...
  if (v === 'estadisticas' && !diario.value && !cargandoDiario.value) {
    fetchDiario();
