      ...
      <div class="flex-1">
          <div class="hoja-card overflow-hidden">
            <div class="px-lg py-md border-b border-[var(--border-color)] flex flex-wrap justify-between items-center gap-md bg-[var(--surface-color)]/50">
              ...
              <button
                @click="pestana === 'usuarios' ? (fetchUsers(), cargarEstadoDisponibilidad()) : (pestana === 'planes' ? fetchPlanes() : (pestana === 'bloqueos' ? fetchBloqueos() : estadisticasSectionRef?.refrescar()))"
                :disabled="isLoading"
            ...
            <!-- ══ Estadísticas ══ -->
            <template v-else>
              <EstadisticasSection ref="estadisticasSectionRef" :planes="planes" />
            </template>
          </div>
      </div>
...
/* ─── Gestion de planes ───────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
const pestana = ref<'usuarios' | 'planes' | 'bloqueos' | 'estadisticas'>('usuarios');
const pestanas = [
  { valor: 'usuarios' as const, texto: 'Usuarios' },
  { valor: 'planes' as const, texto: 'Planes' },
  { valor: 'bloqueos' as const, texto: 'Bloqueos' },
  { valor: 'estadisticas' as const, texto: 'Estadísticas' }
];
/** Referencia al componente hijo, para que "Refrescar" pueda pedirle que recargue. */
const estadisticasSectionRef = ref<InstanceType<typeof EstadisticasSection> | null>(null);
...
/**
 * Estado activo/deshabilitado: lo sigue usando la pestaña de Usuarios (ver
 * cargarEstadoDisponibilidad/alternarEstadoUsuarioLista mas abajo). La
 * pestaña de Estadisticas tiene su propia copia dentro de
 * EstadisticasSection.vue, que vive aparte por ser una funcionalidad
 * independiente con sus propios dos endpoints.
 */
...
// Estadisticas resuelve la suya propia: EstadisticasSection se monta solo
// cuando pestana vale 'estadisticas' y se autocarga al crearse.
watch(pestana, (v) => {
