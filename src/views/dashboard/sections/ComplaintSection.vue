...
const executeSearch = async () => {
  ...
  if (!cleanCi) {
    errorMsg.value = 'Ingrese un número de cédula.';
    return;
  }
  if (cleanCi.length !== 10) {
    errorMsg.value = 'La cédula debe tener 10 dígitos.';
    return;
  }
  ...
  try {
    const data = await apiService.getComplaint(cleanCi);
    if (!data || (Array.isArray(data) && data.length === 0)) {
        throw new Error('No se registraron denuncias para esta cédula.');
    }
