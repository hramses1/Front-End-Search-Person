<template>
  <ServiceSection fuente="Sistema Público de Salud" :isLoading="isLoading" :errorMsg="errorMsg" :results="resultsData" @search="executeSearch" @copy="copyResults">
    <template #fields>
      <div class="relative group input-container">
        <input v-model="ci" type="text" id="ci_medical" placeholder=" " maxlength="10" class="custom-input peer" />
        <label for="ci_medical">Cédula del paciente (ej. 1712345678)</label>
      </div>
    </template>
...
const executeSearch = async () => {
  ...
  try {
    const data = await apiService.getMedicalAppointments(cleanCi);
    resultsData.value = data;
    emit('refresh-stats');
  } catch (err: any) {
    // La cuota se reembolsa en algunos errores (400/451/500/503); hay que releerla.
    emit('refresh-stats');
    errorMsg.value = err.response?.data?.detail || 'No se encontraron citas médicas.';
  } finally {
