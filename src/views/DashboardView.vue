<template>
  <div class="dashboard-wrapper font-primary min-h-screen flex overflow-hidden relative selection:bg-[var(--accent-color)]/30">
    ...
    <!-- Sidebar -->
    <aside 
      :class="['fixed inset-y-0 left-0 z-40 w-64 transition-all duration-base lg:relative lg:translate-x-0 flex flex-col h-screen hoja-panel', isSidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full lg:translate-x-0']"
    >
      ...
      <div class="flex-1 overflow-y-auto p-md space-y-md custom-scrollbar">
        ...
        <div v-if="isAdmin" class="mb-md px-xs">
          ...
          <button @click="router.push('/admin')" class="w-full flex items-center gap-sm px-md min-h-[2.75rem] rounded-base border border-amber-500/20 bg-amber-500/5 text-amber-500 hover:bg-amber-500/10 transition-all text-caption font-bold tracking-wider">
    ...
    <main id="contenido" class="flex-1 flex flex-col h-screen h-[100dvh] relative z-10 p-lg sm:p-xl overflow-y-auto custom-scrollbar animate-aparecer">
      ...
      <header class="mb-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-md pr-3xl lg:pr-0">
        ...
        <div class="flex flex-wrap items-center justify-end gap-sm w-full md:w-auto md:gap-md self-end md:self-auto">
          ...
          -->
          <div class="control-encabezado hoja-panel">
            ...
            <div class="flex flex-col leading-none">
              <!--
                leading-none en ambas: text-caption trae interlineado 1.5, y dos
                lineas asi sumaban mas que la altura de la fila.
              -->
...
import CivilStatusSection from './dashboard/sections/CivilStatusSection.vue';
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'mother', label: 'Nombre de la madre', titulo: 'Nombre de la madre',
        icon: ['M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'] },
      { key: 'civilstatus', label: 'Estado civil', titulo: 'Estado civil',
        icon: ['M16.5 3.75a2.25 2.25 0 100 4.5 2.25 2.25 0 000-4.5zM7.5 15.75a2.25 2.25 0 100 4.5 2.25 2.25 0 000-4.5zM14.834 8.628L9.166 15.372'] }
    ]
  },
  {
    label: 'Judicial',
...
const activeComponent = computed(() => {
  const components: any = {
    ...
    civilstatus: CivilStatusSection
  };
