<template>
  <footer class="mt-auto pt-2 border-t border-[var(--border-color)] flex flex-col sm:flex-row items-center justify-between gap-3 pb-3 relative">
    <!-- Copyright & Legal -->
    <div class="flex items-center gap-3 order-2 sm:order-1 flex-wrap justify-center">
      <span class="text-[10px] font-black opacity-60 tracking-widest uppercase">© {{ anio }} Devzio</span>
      ...
      <button
        ...
        class="text-[10px] font-bold tracking-[0.15em] uppercase opacity-40 hover:opacity-100 hover:text-[var(--accent-color)] transition-all"
      >
        {{ l.texto }}
      </button>
    </div>
